...
```{r class-phylum-stats-3, echo=T, eval=T}
# Is this valid???
...
```{r ecotypes-cortest, echo=F, eval=F}
...
pw_dist2 <- function(ali, anno){
  ...
  anno["cat"] <- c("none")
  anno$cat[anno$fdr<0.05 & anno$classification=="decreasing"] <- c("shallow") # can be set to "inc_dec_p_val" instead for less stringency
  anno$cat[anno$fdr<0.05 & anno$classification=="increasing"] <- c("deep") # can be set to "inc_dec_p_val" instead for less stringency
  anno <- anno[,c("OTU_num", "cat")]
  anno$OTU_num <- as.numeric(anno$OTU_num)
  colnames(anno) <- c("Var1", "Var1_cat")
  dist <- left_join(dist, anno)
...
```{r sisterOTU-viz, echo=T, eval=T, fig.asp=1.5, fig.cap="Relative abundance of all sister OTUs in _G. barretti_ and _S.fortis_. Note that only the category \"both\" and the respective sponge host (gb or sf) are relevant. The OTUs classified as sister OTUs in the other sponge are added for comparison only."}
