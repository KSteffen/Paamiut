...
```{r diversities, fig.cap="Microbiota diversity indices grouped by sponge species and ordered by sample depth."}
library(vegan)
library(reshape2)
library(phyloseq)
library(picante)
...
ggplot(df_phylum, aes(x=Sample_ID, y=variable, fill=Phylum))+
...
Summarising the OTU diversity by phylum or class only show what was previously known. The composition of the sponges' prokaryotic community is stable. However, summarising doesn't do the data justice. We therefore correlate the average relative abundance of every OTU with depth
