...
### dfs to saving, join _reponse to rabdc_ and calculate avg </>1000 m
...
#write.csv(gb_OTUs, "data/gb_OTUs_overall_rabdc_annotated.csv") #second option overal_rabdc
...
## Sister OTU visualisation
## OBS OBS OBS an OTU can be present in Gb and Sf but only be classified as "sister" in one of them! Show all but make clear which one is sis and which one isn't. Gb has 30 sis OTUs, Sf has 9 (Wb: 0, will be ignored)
...
## Gb
gb_overall_rabdc <- read.csv("data/gb_OTUs_overall_rabdc_annotated.csv", header = T, sep = ",")
rownames(gb_overall_rabdc) <- gb_overall_rabdc$X
gb_overall_rabdc$X <- NULL
gb_overall_rabdc["OTU_num"] <- str_replace(gb_overall_rabdc$XOTU,"X196900", "")
gb_sis <- left_join(sisters, gb_overall_rabdc, by=c("sis"="OTU_num"))
...
## SF
sf_overall_rabdc <- read.csv("data/sf_OTUs_overall_rabdc_annotated.csv", header = T, sep = ",")
rownames(sf_overall_rabdc) <- sf_overall_rabdc$X
sf_overall_rabdc$X <- NULL
sf_overall_rabdc["OTU_num"] <- str_replace(sf_overall_rabdc$XOTU,"X196900", "")
