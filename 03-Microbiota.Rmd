...
#ggplot(micro_pca_df, aes(x=PC1, y=PC2))+geom_point(size=3, mapping=aes(shape=factor(Species), col=Depth))+ggtitle(my_title)+xlab(x1)+ylab(y1)+labs(shape="Species")+theme_bw()+theme(legend.position = "none")
...
#with(micro_pca_df, scatter3D(x = PC1, y = PC2, z = -PC3, colvar =micro_pca_df$Depth, pch = 16, cex = 1.5, xlab =x1, ylab =y1, zlab =z1, clab = c("Depth","m"), main = "3D microbiome PCA", ticktype = "detailed", theta = 10, d = 2, colkey = list(length = 0.5, width = 0.5, cex.clab = 0.75)))

#ks1
#scatter3D(x=micro_pca_df$PC1, y=micro_pca_df$PC2, micro_pca_df$PC3) #works

#crude #KEEP
#scatter3D(x=micro_pca_df$PC1, y=micro_pca_df$PC2, micro_pca_df$PC3, phi = 0, bty = "g",pch = 20, cex = 2, ticktype = "detailed")

#scatter3D(x=micro_pca_df$PC1, y=micro_pca_df$PC2, micro_pca_df$PC3, phi = 0, labels = micro_pca_df$unified_ID, bty = "g",pch = 20, cex = 2, ticktype = "detailed",colvar = micro_pca_df$Depth)
...
```{r envfit-1, fig.cap="Fitting significant (p<0.05) environmental vectors onto ordination of G. barretti microbiome."}
microbiome<-read.csv("data/OTU_all_R.csv", header=T, sep=";") 
meta_data <- read.csv("data/Steffen_et_al_metadata_PANGAEA.csv", header=T, sep=";")
...
```{r envfit-2, fig.cap="Fitting significant (p<0.05) environmental vectors onto ordination of S. fortis microbiome."}
...
```{r envfit-3, fig.cap="Fitting significant (p<0.05) environmental vectors onto ordination of W. bursa microbiome."}
...
"unclassified", "Verrucomicrobia"),
                  ...
                  labels = c("Acidobacteria", "Actinobacteria", "Bacteroidetes", "Chlamydiae", "Chloroflexi", "Cyanobacteria", "Deferribacteres", "Deinococcus.Thermus", "Firmicutes", "Gemmatimonadetes", "Nitrospinae", "Nitrospirae", 
                             "PAUC34f", "Planctomycetes", "Poribacteria", "Proteobacteria", "SBR1093", "Spirochaetae", "Tectomicrobia", "Thaumarchaeota", 
                             "unclassified", "Verrucomicrobia")
                  )
...
#grid.arrange(stacked, perc, nrow=2)
...
unique(yes_inc$name) # To annotate manually in the figure "148" "164" "138" "156" "337" "242" "236" "339" "465" "454" "308" "151" "269" "229"
...
yes_dec <- decrease %>%
  na.omit() %>%
  filter(spongeEMP_enriched=="yes")
unique(yes_dec$name) # To annotate manually in the figure: "215" "176" "180" "209" "289" "192" "182" "183" "335" "146" "196" "211" "161" "260" "277" "320" "144" "213" "331" "310" "336" "589" "588" "189" "137"
length(unique(yes_dec$name)) #25
...
In this Fig.\@ref(fig:increade-decrease) as well as in Tab. \@ref(tab:increade-decrease), we see that at the OTU level, we observe shifts. While sometimes more gradual, there seem to OTUs exclusievely present in the "shallow" or the "deep" samples in all three sponges. In fact, in _G. barretti_ and _S. fortis_, the number of OTUs increasing with depth ("deep water mass microbiome") is greater than the number of OTUs decreasing. This leads us to believe that the deep water mass contains microbes yet to discover.

```{r OTU-depth-viz, echo=FALSE, eval=FALSE}
...
averages_gb <- cbind(shallow_gb, deep_gb, ttest_gb)
averages_sf <- cbind(shallow_sf, deep_sf, ttest_sf)
averages_wb <- cbind(shallow_wb, deep_wb, ttest_wb)

colnames(averages_gb) <- c("shallow_mean", "deep_mean", "ttest_pval")
colnames(averages_sf) <- c("shallow_mean", "deep_mean", "ttest_pval")
colnames(averages_wb) <- c("shallow_mean", "deep_mean", "ttest_pval")
...
#write.csv(gb_OTUs, "data/gb_OTUs_overall_rabdc_annotated_.csv") #second option overal_rabdc
#write.csv(sf_OTUs, "data/sf_OTUs_overall_rabdc_annotated.csv")
#write.csv(wb_OTUs, "data/wb_OTUs_overall_rabdc_annotated.csv")

### For Paco's OTUs, REMOVE LATER
gb_pc <- rbind(rabdc$gb["X196900146",], rabdc$gb["X196900138",], rabdc$gb["X1969004",], rabdc$gb["X19690097",], rabdc$gb["X196900575",], rabdc$gb["X196900144",], rabdc$gb["X196900588",], rabdc$gb["X196900337",], rabdc$gb["X196900578",], rabdc$gb["X196900137",], rabdc$gb["X196900142",], rabdc$gb["X196900165",], rabdc$gb["X196900309",])
gb_pc <- data.frame(t(gb_pc))
gb_pc["unified_ID"] <- rownames(gb_pc)
gb_pc[, c("NA.", "NA1", "NA2")] <- list(NULL)
gb_pc <- reshape2::melt(gb_pc, idvars=c("unified_ID"))
md_pc_g <- meta_data[, c("unified_ID", "Depth")]
gb_pc <- left_join(gb_pc, md_pc_g)
ggplot(gb_pc, aes(x=Depth, y=value))+geom_point()+facet_wrap(~variable, scales="free")

sf_pc <- rbind(rabdc$sf["X196900146",], rabdc$sf["X196900138",], rabdc$sf["X1969004",], rabdc$sf["X19690097",], rabdc$sf["X196900575",], rabdc$sf["X196900144",], rabdc$sf["X196900588",], rabdc$sf["X196900337",], rabdc$sf["X196900578",], rabdc$sf["X196900137",], rabdc$sf["X196900142",], rabdc$sf["X196900165",], rabdc$sf["X196900309",])
sf_pc <- data.frame(t(sf_pc))
sf_pc["unified_ID"] <- rownames(sf_pc)
sf_pc[, c("NA.", "NA1")] <- list(NULL)
sf_pc <- reshape2::melt(sf_pc, idvars=c("unified_ID"))
md_pc_g <- meta_data[, c("unified_ID", "Depth")]
sf_pc <- left_join(sf_pc, md_pc_g)
ggplot(sf_pc, aes(x=Depth, y=value))+geom_point()+facet_wrap(~variable, scales="free")
...
```{r produce-fastas, echo=F, eval=F}
library(seqinr)
library(tidyverse)
#library(dplyr)

setwd("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/seqs/")

### FASTA FILE GENERATION

# load data
fastas <- read.csv("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/xcms_publication/OTUs_reads_fasta_column.csv", header=F, sep=";")
head(fastas)

#OTU_annotation <- read.csv("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/xcms_publication/heatmap_figure_gb.csv", header=T, sep=",") # for Gb
OTU_all <- read.csv("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/Microbiome/OTU_all_R.csv", header = T, sep = ";")
...
XOTU <- data.frame(colnames(OTU_sf))
XOTU <- data.frame(colnames(OTU_wb))
...
#======================= produce fasta fortmatted output file 
# CHANGE NAME for the different data set

sink("gb_seqs.txt")
n <- 0
k <- dim(fastas)[1]
while(n<=k){
  cat(paste(fastas$concat[n], fastas$newline[n], fastas$seq[n], fastas$newline[n]))
  n <- n+1
}
sink()
...
#Split fasta subsets here in terminal, like so:
# $ cd to wd()
# $ grep ">" wb_seqs.txt > wb_seqs_header.txt
# $ grep -v ">" wb_seqs.txt > wb_seqs_fasta.txt


#======================= Labelled fasta files for phylogenetics =====
# FUN combining data sets
fasta_headers <- function(seqs_header, seqs, taxonomy, response){
  colnames(seqs) <- c("seqs")
  fasta_phylogeny <- bind_cols(seqs_header, seqs)
  fasta_phylogeny["num"] <- as.numeric(str_sub(fasta_phylogeny$V1,2,5))
  response["num"] <- as.numeric(str_sub(response$XOTU, -3))
  fasta_phylogeny <- left_join(fasta_phylogeny, response[, c("classification", "num")])
  taxonomy["num"] <- as.numeric(str_sub(taxonomy$OTU_ID, -3))
  tax <- taxonomy[taxonomy$num %in% fasta_phylogeny$num,]
  fasta_phylogeny <- left_join(fasta_phylogeny, tax[, c("Phylum", "Class", "num")])  
  fasta_phylogeny["header"] <- paste(fasta_phylogeny$V1, fasta_phylogeny$classification, fasta_phylogeny$Phylum, fasta_phylogeny$Class, sep="_")
  fasta_phylogeny[, c("V1","Class","classification", "num", "Phylum")]<- list(NULL)
  fasta_phylogeny["newline"] <- "\n"
  return(fasta_phylogeny)  
}

# load one set at a time
seqs_header <- read.csv("gb_seqs_header.txt", header=F)
seqs <- read.csv("gb_seqs_fasta.txt", header=F)
response <- read.csv("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/Microbiome/gb_response.csv")

seqs_header <- read.csv("sf_seqs_header.txt", header=F)
seqs <- read.csv("sf_seqs_fasta.txt", header=F)
response <- read.csv("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/Microbiome/sf_response.csv")

seqs_header <- read.csv("wb_seqs_header.txt", header=F)
seqs <- read.csv("wb_seqs_fasta.txt", header=F)
response <- read.csv("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/Microbiome/wb_response.csv")

# load
taxonomy <- read.csv("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/xcms_publication/microbiome_taxonomy.csv", header = T, sep = ";")

# RUN  combining data sets
fasta_phylogeny <- fasta_headers(seqs_header, seqs, taxonomy, response)

# save to file

sink("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/seqs/gb_reads_for_phylogeny.fasta")
sink("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/seqs/sf_reads_for_phylogeny.fasta")
sink("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/seqs/wb_reads_for_phylogeny.fasta")

n <- 0
k <- dim(fasta_phylogeny)[1]
while(n<=k){
  cat(paste(fasta_phylogeny$header[n], fasta_phylogeny$newline[n], fasta_phylogeny$seqs[n], fasta_phylogeny$newline[n]))
  n <- n+1
}
sink()

# The files have excess [spaces] that can be removed by hand in any text processing programme.
```

We produced fasta files, i.e. files containing the DNA sequences of the OTUs in the three sponges. The fasta header contains the following information: "OTU number"\_"depth response"\_"phylum"\_"class". The depth response is coded as "increasing" or "decreasing" if there is a significant correlation (_p_$\leq$ 0.05) with depth and as "inc. trend" or "dec. trend" if the correlation is not significant (_p_$\geq$ 0.05).
The fasta sequences were aligned with [MAFFT](https://mafft.cbrc.jp/alignment/server/). From the aligned seqeunces, we calculated the sequence similarity comparing all versus all, and retain in a file all comparisons yielding a sequence similarity $\geq$ 97%.

```{r sequence-similarity-1, eval=F}
...
write.csv(dist, "~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/Phylogeny/wb_similarity.csv") # WB
```

```{r sequence-similarity-2, echo=F, eval=T}
dist_gb <- read.csv("data/gb_similarity.csv")
dist_sf <- read.csv("data/sf_similarity.csv")
dist_wb <- read.csv("data/wb_similarity.csv")
```
In _G. barretti_ there are `r dim(dist_gb)[1]` pairs of OTUs with a sequence similarity $\geq$ 97% and opposing responses to depth, in _S. fortis_ there are `r dim(dist_sf)[1]` pairs while in _W. bursa_, there are none.

## Phylogeny

The MAFFT aligend fasta sequences of _G. barretti_ OTUs were used to infer a phylogeny under maximum likelihood model in RaxML/Bayesian statistics.
...
```{r sequence-similarity, echo=F, eval=F}
library(seqinr)
library(tidyverse)
#library(dplyr)
...
# For similarity calculations, the fasta files need to be ALIGNED. This can be done manually in e.g. with MAFFT (https://mafft.cbrc.jp/alignment/server/). KS 20190829: MAFFT (v7.452)
# load data
#gb <- read.alignment("GB_reads_annotated.fasta", "fasta")
#gb <- read.alignment("gb_127.fasta", "fasta")
#gb

ali <- read.alignment("gb_reads_for_phylogeny_MAFFT.fasta", "fasta")
ali <- read.alignment("sf_reads_for_phylogeny_MAFFT.fasta", "fasta")
ali <- read.alignment("wb_reads_for_phylogeny_MAFFT.fasta", "fasta")

# FUN calculate pairwise distance, melt, kepps only entries with >97% and <1 similarity with significant opposing trends in both partners.
pw_dist <- function(ali){
  dist <- as.matrix(dist.alignment(ali, matrix = "similarity" ))  #https://www.researchgate.net/post/Homology_similarity_and_identity-can_anyone_help_with_these_terms
  # dist.alignment: matrix contains the squared root of the pairwise distances. For example, if identity between 2 sequences is 80 the squared root of (1.0 - 0.8) i.e. 0.4472136.
  dist <- dist**2
  ...
  dist["Var2_response"] <- str_sub(dist$Var2, 5,10)
  #remove not significant comparisons
  #dist <- dist[!dist$Var1_response=="dec.tr",]
  #dist <- dist[!dist$Var1_response=="inc.tr",]
  #dist <- dist[!dist$Var2_response=="dec.tr",]
  #dist <- dist[!dist$Var2_response=="inc.tr",]
  dist <- dist[!dist$Var2_response==dist$Var1_response,]
    # order
  dist <- dist[order(dist$Var1_response, dist$Var2_response),]
  return(dist)
...
dist
dim(dist)[1]
aggregate(dist, by=list(dist$X1_response, dist$X2_response), length)
