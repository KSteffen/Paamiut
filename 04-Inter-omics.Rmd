...
write.csv(cor_file_shortened, "SparCC_for_networks.csv")
...
#write.csv(e_values, "MIC_e_values.csv")
```
...
#write.csv(cor_file_shortened, "MIC_for_networks.csv")
```
...
#e.g. metap::sumlog, or EmpiricalBrownsMethod::EBM
library(metap)

ms <- read.csv("data/master_summary_networks_1.csv", header=T)
head(ms)

#For metap::sumlog, replace all zeros with small non-zero values. E.g. half-minimum
par(mfrow=c(2,3))
hist(ms$pMIC)
...
#sumlog doesn't think 0 is a valid p-value
#min(ms$pMIC, na.rm=T)
#max(ms$pMIC, na.rm=T)
#min(ms$pLSA, na.rm=T)
#max(ms$pLSA, na.rm=T)
#min(ms$pSparCC, na.rm=T)
#max(ms$pSparCC, na.rm=T)
#min(ms$pMENA_PCC, na.rm=T)
#max(ms$pMENA_PCC, na.rm=T)
#min(ms$pMENA_SCC, na.rm=T)
#max(ms$pMENA_SCC, na.rm=T)
...
#write.csv(ms, "master_summary_networks_p-val.csv")

#setwd("~/Documents/Metabolomics/Depth_Gradient_study/R_depth_study/Network_analyses_R")
#ms <- read.csv("master_summary_networks_p-val.csv", header=T, sep=",") #NEW START HERE
#rm(lsa, mena_pcc, mena_scc, mic, sparcc)

hist(ms$NA_count, main="NAs", xlab="Count per edge ", breaks = c(0,1,2,3,4,5), labels = TRUE)
hist(ms$signif_0.05, main="p-values <= 0.05", xlab="Count per edge ", breaks = c(0,1,2,3,4,5), labels = TRUE) # SELECTION/INCLUSION CRITERION
hist(ms$signif_0.001, main="p-values <= 0.001", xlab="Count per edge ", breaks = c(0,1,2,3,4,5), labels = TRUE)
```

We select all edges with at least 4 p-values $\leq$ 0.05 (n=`r sum(ms$NA_count==1)+sum(ms$NA_count==0)`) to visualise in the network.

```{r igraph1, echo=TRUE, eval=TRUE, fig.align='center'}
#head(ms)
ds <- split(ms, ms$signif_0.05)
raw_nodes <- rbind(ds$`4`, ds$`5`) # SELECTION/INCLUSION CRITERION
rm(ds)

# check ms for archaea: X196900146, X196900359
head(ms)
top1 <- ms[ms$node1_mic=="OTU196900146",]
bottom1 <- ms[ms$node2_mic=="OTU196900146",]
top2 <- ms[ms$node1_mic=="OTU196900359",]
bottom2 <- ms[ms$node2_mic=="OTU196900359",]
archaea <- rbind(top1, top2, bottom1, bottom2)
archaea <- archaea[order(archaea$NA_count),]
#


par(mfrow=c(2,2))
hist(raw_nodes$sumlog)
hist(ms$sumlog)
hist(ms$p.adjust_Bonferroni)
hist(ms$p.adjust_FDR)
par(mfrow=c(1,1))
#nrow(ms)#41616
#sum(ms$p.adjust_Bonferroni<=0.001, na.rm=T) #39
#sum(ms$p.adjust_Bonferroni<=0.05, na.rm=T) #168
#sum(ms$sumlog<=0.001, na.rm=T) #1446
